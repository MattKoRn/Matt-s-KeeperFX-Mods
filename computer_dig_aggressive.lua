...
-- Makes PLAYER0 auto-dig toward hero fortress hearts, rival keeper hearts,
-- hero gates, and the lord of the land.
...
ComputerDigAggressive.INTERVAL = ComputerDigAggressive.INTERVAL or 3000
-- Dig toward rival keeper hearts too.
ComputerDigAggressive.DIG_TO_RIVAL_KEEPERS = ComputerDigAggressive.DIG_TO_RIVAL_KEEPERS ~= false
-- Try to find hero gates to dig toward.
ComputerDigAggressive.DIG_TO_HERO_GATES = ComputerDigAggressive.DIG_TO_HERO_GATES ~= false
...
local function is_hero_gate(thing)
    local model = tostring((thing and thing.model) or ""):upper()
    return model == "HERO_GATE" or model == "HEROGATE" or model == "HERO GATE"
end

local function add_hero_gate_targets(targets)
    if not ComputerDigAggressive.DIG_TO_HERO_GATES or type(GetThingsOfClass) ~= "function" then return end

    local ok, things = pcall(GetThingsOfClass, "Object")
    if not ok or type(things) ~= "table" then return end

    for _, thing in ipairs(things) do
        if is_hero_gate(thing) and thing.stl_x and thing.stl_y then
...
local function get_targets_for(digger)
    ...
    add_hero_gate_targets(targets)
    return targets
end

local function dig_to_target(target)
    if type(ComputerDigToLocation) ~= "function" or not PLAYER0 then return false end

    if type(target) == "table" and target.is_location then
        return pcall(ComputerDigToLocation, PLAYER0, PLAYER0, target.stl_x, target.stl_y)
    end

    return pcall(ComputerDigToLocation, PLAYER0, PLAYER0, target)
end
...
function ComputerDigAggressive_Init()
    ...
    if type(RegisterTimerEvent) ~= "function" then return end
    if type(ComputerDigToLocation) ~= "function" then return end

    local ok = pcall(RegisterTimerEvent, "ComputerDigAggressive_OnTick", ComputerDigAggressive.INTERVAL, true)
    if ok then
